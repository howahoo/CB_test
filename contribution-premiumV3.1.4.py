...
def create_visualization(df):
    ...
    # 用时间戳作为z轴
    z_time = df['eob'].astype(np.int64)
    z_time_str = df['eob'].dt.strftime('%Y-%m-%d %H:%M') if hasattr(df['eob'], 'dt') else df['eob'].astype(str)
    # 采样10个点用于z轴刻度
    sample_idx = np.linspace(0, len(z_time)-1, 10, dtype=int)
    sampled_timestamps = z_time.iloc[sample_idx]
    ...
    fig3d.update_layout(
        scene=dict(
            ...
            zaxis=dict(
                ...
                tickfont=dict(size=12),
                title_font=dict(size=14),
                tickvals=sampled_timestamps,
                ticktext=sampled_time_strs,
                range=[z_time.min(), z_time.max()]
            ),
        ),
        title=dict(
    ...
    z_range = np.linspace(df['eob'].min().timestamp(), df['eob'].max().timestamp(), 30) # 使用时间戳作为z轴范围
...
def create_3d_visualization_both(df):
    ...
    fig.update_zaxes(
        ...
        dict(bounds=[15, 24], pattern="hour"),
